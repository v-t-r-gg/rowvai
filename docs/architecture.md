...
Migration 6 adds append-only invalidations, immutable resolved dataset manifests, and immutable analysis runs. Analysis verifies sealed evidence and scorer revision 1, records exact inclusion/exclusion, computes versioned quality and calibration diagnostics, and persists canonical input/report digests. SQLite triggers prevent mutation or deletion of these evidence tables. Metadata export omits value-bearing bundles; explicit full-local export remains file-only in the CLI. None of these services depend on or modify authorization policy.
