...
The record revision must equal the original receipt's post-commit revision and every affected value must still equal the original `after` value. Rowva previews an inverse update with the exact `before` values and current revision witness. The inverse has its own ID and fingerprint and links through `reverts_operation_id`.
